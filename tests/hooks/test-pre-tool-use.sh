#!/usr/bin/env bash
# Integration test for hooks/pre-tool-use.sh
set -euo pipefail
...
echo "==> Sending synthetic PreToolUse payload (session_id=${TEST_SESSION_ID})..."
...
[ "${COUNT}" -ge 1 ] && [ "${EVENT}" = "PreToolUse" ] \
  && echo "PASS: PreToolUse point found (session_id=${TEST_SESSION_ID})" \
  || { echo "FAIL: expected 1+ PreToolUse points, got ${COUNT} (event_type=${EVENT})"; exit 1; }
