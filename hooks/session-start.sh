...
if [[ -n "${BRIEF_ID:-}" && ! -f "${SCRIPT_DIR}/session-materialise.py" ]]; then
  ...
  echo "⛔ session-materialise.py MISSING but BRIEF_ID=${BRIEF_ID} is set — the brief-first" \
       "session cannot materialise. HALT: this hook set is incompletely synced" \
       "(session-materialise.py must be resident). Do NOT begin tasking; re-sync and re-launch." >&2
